...
public class DiemdanhBrowse extends StandardLookup<Diemdanh> {
    ...
    protected void onTimkiemClick(Button.ClickEvent event) {
        ...
        if (tengiaovienField.getValue() == null) {
            String loaddonvi = "select e from truonghoc_Diemdanh e where e.donvitao_diemdanh = :donvi";
            diemdanhsDl.setQuery(loaddonvi);
            if (ngaylamField.getValue() != null) {
                String loadngaylam = "select e from truonghoc_Diemdanh e where e.ngaynghi = :ngaynghi and e.donvitao_diemdanh = :donvi";
                diemdanhsDl.setQuery(loadngaylam);
            }
        } else {
            String loadhoten = "select e from truonghoc_Diemdanh e where e.donvitao_diemdanh = :donvi and e.nguoitaodd = :nguoitao";
            diemdanhsDl.setQuery(loadhoten);
            if (ngaylamField.getValue() != null) {
                String loadngaylam = "select e from truonghoc_Diemdanh e where e.ngaynghi = :ngaynghi and e.donvitao_diemdanh = :donvi and e.nguoitaodd = :nguoitao";
                diemdanhsDl.setQuery(loadngaylam);
            }
        }
